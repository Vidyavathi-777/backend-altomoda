...
const Header = ({ currentGender, onGenderChange }) => {
  ...
  const cssVariables = {
    ...
    fontAccent: "'Didot', serif",
  };

  return (
    <header
      className="w-full bg-white font-[var(--font-body)]"
    ...
    >
      ...
      <div className="bg-black text-white text-center font-bold py-3 px-4 hidden lg:flex justify-between items-center text-4xl sm:text-xs">
        <div
          className="text-left font-light px-8"
          style={{ fontFamily: cssVariables.fontAccent }}
        >
          INDIA - EN - <span>₹</span>
        </div>
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4">
          {/* Mobile Header */}
          <div
            className={`flex items-center justify-between py-4 lg:hidden bg-white transition-all duration-300 ${
              isScrolled
                ? "fixed top-0 left-0 right-0 z-50 px-4 shadow-md"
                : ""
            }`}
          >
      ...
      {/* Simple Scale Down Animation Styles */}
      <style jsx>{`
        @keyframes scaleDown {
          0% {
            transform: scale(1.3);
        ...
        .animate-scale-down-fast {
          animation: scaleDownFast 0.6s ease-out forwards;
        }
      `}</style>
    </header>
  );
