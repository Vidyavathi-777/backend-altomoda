...
const TryOnModal = ({ open, onClose, productImage, sku }) => {
  ...
  const handleTryOn = async () => {
    ...
    try {
      ...
      // Extract parentSku safely from productImage object
      let parentSku = sku

      if (!parentSku) {
        throw new Error("Product parent SKU is missing");
      }
      ...
      if (response.data.success) {
        let img = response.data.tryonImage;
        img = img.replace(/^"|"$/g, "");   // <-- FIX
                  ...
                  >
                    <div className="relative">
                      <img
                        src={generatedImage}
                        alt="Try On Result"
                        className="w-full h-100 object-contain rounded-xl bg-gray-50 p-4 shadow-sm border-2 border-green-200 group-hover:shadow-md transition-shadow duration-200"
                      />
